(function () {
  ...
  function isTizen60Chrome76() {
  var u = UA();
  return /Tizen 6\.0/i.test(u) && /76\.0\.3809\.146\/6\.0/i.test(u);
  }
  ...
  function isTvDevice() {
    return /SMART-TV|Tizen|Web0S|NetCast|HbbTV|AndroidTV|Android TV|BRAVIA|AFT|MiBOX|Mi Box|MIBOX|Xiaomi|TV/i.test(UA());
  }
  ...
  function detectPlayerOpen() {
    ...
    // In Lampa builds (Android TV / Mi Box) a <video> element can exist for previews
    // and would cause false positives. Prefer detecting the real player container.
    if (window.Lampa) {
      var pe = null;
      try { pe = document.querySelector('.player, .player__layer, .player__video, .player-panel, .player__body, .player__content'); } catch (e) {}
      if (pe && isElVisible(pe)) {
        var pr = pe.getBoundingClientRect();
        if (rectAreaRatio(pr) >= 0.10) return true;
      }

      var pv = null;
      try { pv = document.querySelector('.player video, video.player, .player__video video'); } catch (e2) {}
      if (pv && isElVisible(pv)) {
        var vr = pv.getBoundingClientRect();
        if (rectAreaRatio(vr) >= 0.20) return true;
      }

    // Fallback (non-Lampa pages)
...
function addSettingsUI() {
  ...
  function applyMode() {
    ...
    try {
      ...
      try { current_renderer = createDomRenderer(q); } catch (e2) { current_renderer = null; }
    }
  }
  ...
  function createCanvasRenderer(Q) {
    ...
    var TV = isTvDevice();
    var MOBILE = isMobileDevice();
	var TIZEN6_FIX = isTizen60Chrome76();
	var safeL = 0, safeR = 0; // зарезервированные зоны слева/справа (px)
    ...
    var ctx = null;
    try { ctx = canvas.getContext('2d', { alpha: true }) || canvas.getContext('2d'); } catch (e) { ctx = null; }

    // If canvas context is unavailable (rare on some WebViews), fallback to DOM renderer
    if (!ctx) {
      try { if (root && root.parentNode) root.parentNode.removeChild(root); } catch (e2) {}
      try { var s0 = document.getElementById(CANVAS_STYLE_ID); if (s0 && s0.parentNode) s0.parentNode.removeChild(s0); } catch (e3) {}
      return createDomRenderer(Q);
    }
    ...
    function setupCanvasSize() {
      ...
      var w = Math.max(320, window.innerWidth || 1920);
	  if (!MOBILE && TIZEN6_FIX) {
	  // адаптивно под разную ширину ТВ
	  safeL = Math.max(90, Math.min(180, Math.round(w * 0.08)));  // под логотип/левые иконки
	  safeR = Math.max(260, Math.min(420, Math.round(w * 0.20))); // под часы/правые иконки
	} else {
	  safeL = 0;
	  safeR = 0;
	}
      ...
      buildBulbs(w, safeL, safeR);
    }

	function buildBulbs(w, safeL, safeR) {
  bulbs = [];

  var usableW = Math.max(240, w - (safeL || 0) - (safeR || 0));
  var count = getBulbCount(usableW);

  var wireY = MOBILE ? 36 : ((TIZEN || TV) ? 34 : 72);
  ...
  var spacing = (count <= 1) ? 0 : (usableW / (count - 1));
  var startX = (safeL || 0);
  ...
  for (var i = 0; i < count; i++) {
    ...
    bulbs.push({
      i: i,
      t: t,
      x: startX + i * spacing,  // <-- ВАЖНО: X уже с учётом safeL/safeR

(function () {
  ...
  function isAndroidTV() {
    ...
    if (/MIBOX|MiBox|AFT|BRAVIA|SmartTV|Android TV|Nexus Player|SHIELD/i.test(ua)) return true;
    // иногда Android TV UA содержит "Mobile", поэтому опираемся на модель/бренд в UA
    if (/\bTV\b/i.test(ua) && /Android/i.test(ua)) return true;
    try {
      if (window.Lampa && Lampa.Platform && Lampa.Platform.is) {
        // некоторые сборки имеют отдельный флаг tv
        if (Lampa.Platform.is('android') && (Lampa.Platform.tv || Lampa.Platform.is('android_tv'))) return true;
      }
    } catch (e) {}
    return false;
  }
...
function isElVisible(el) {
 ...
 function detectOverlayOpen() {
  ...
  for (var i = 0; i < sels.length; i++) {
    ...
    // Частая проблема на Android TV/WebView: контейнеры уведомлений/слоёв могут быть всегда в DOM
    // (например .notification), занимать весь экран, но быть "пустыми". Это не должно гасить снег.
    var cs = null;
    try { cs = window.getComputedStyle ? getComputedStyle(el) : null; } catch (e_cs) { cs = null; }
    if (cs && cs.pointerEvents === 'none') continue;

    // Игнорируем пустой контейнер уведомлений (если нет видимого содержимого)
    if (sels[i] === '.notification') {
      try {
        var hasChild = !!(el.children && el.children.length);
        var txt = (el.textContent || '').trim();
        if (!hasChild && !txt) continue;
      } catch (e_nt) {}
    }
  ...
  // В некоторых сборках компоненты отличаются (например Android TV / Mi Box).
  // Чтобы снег не "пропадал" полностью из‑за неизвестного component, разрешаем показывать снег
  // на любых экранах, кроме явно исключённых (плеер/настройки/поиск/клавиатура).
  function isDisallowedComponentName(c) {
    c = (c || '').toString().toLowerCase();
    // список намеренно короткий и безопасный
    return (
      c.indexOf('player') !== -1 ||
  ...
  function isFallbackAllowedActivity(e) {
    var c = (e && (e.component || (e.object && e.object.component))) || '';
    if (!c) return true;
    if (isDisallowedComponentName(c)) return false;
    return true;
  }
  ...
  function computeConfig() {
    ...
    var settleDefault = (tizen || isAndroidTV()) ? 0 : 1;
...
function resetAccumulationSoft(reason) {
  ...
  function get2dCtx(canvas) {
    ...
    try {
      // Некоторые WebView/браузеры (особенно на Android TV) могут возвращать null,
      // если передан второй аргумент с опциями. Делаем безопасный фоллбек.
      return canvas.getContext('2d', { alpha: true }) || canvas.getContext('2d');
    } catch (e) {
      try { return canvas.getContext('2d'); } catch (e2) { return null; }
    }
  }

// Корневой слой поверх интерфейса (на некоторых Android TV обычный appendChild в body может попадать под слои/stacking-context)
var snowfxRoot = null;
function ensureRootLayer() {
  try {
    if (snowfxRoot && snowfxRoot.parentNode) return snowfxRoot;
    var exist = document.getElementById('snowfx_root');
    if (exist) { snowfxRoot = exist; return snowfxRoot; }
    ...
    // Важно: цепляем к documentElement, а не к body (на Android TV body иногда является "слоем" с клиппингом)
...
function ensureCanvases() {
    ...
    if (!fallCanvas) {
      ...
      ensureRootLayer().appendChild(fallCanvas);
      fallCtx = get2dCtx(fallCanvas);
      // Фоллбек для Android TV WebView: иногда контекст отдаётся только после задания размеров
      if (!fallCtx) {
        try { fallCanvas.width = 2; fallCanvas.height = 2; } catch(e){}
        fallCtx = get2dCtx(fallCanvas);
      }
    }
    ...
    if (!accCanvas) {
      ...
      ensureRootLayer().appendChild(accCanvas);
      accCtx = get2dCtx(accCanvas);
      if (!accCtx) {
        try { accCanvas.width = 2; accCanvas.height = 2; } catch(e){}
        accCtx = get2dCtx(accCanvas);
      }
    }
    ...
    // Фоллбек для WebView на Android TV: иногда position:fixed ведёт себя как relative и canvas получается маленьким.
    // Если canvas внезапно маленький — переводим в absolute и цепляем к document.documentElement.
    try {
      if (fallCanvas) {
        var r = fallCanvas.getBoundingClientRect && fallCanvas.getBoundingClientRect();
        if (r && (r.width < 100 || r.height < 100)) {
          ...
          if (fallCanvas.parentNode !== ensureRootLayer()) { try { ensureRootLayer().appendChild(fallCanvas); } catch(e){} }
        }
      }

      if (accCanvas) {
        var r2 = accCanvas.getBoundingClientRect && accCanvas.getBoundingClientRect();
        if (r2 && (r2.width < 100 || r2.height < 100)) {
          accCanvas.style.position = 'absolute';
          accCanvas.style.left = '0';
          accCanvas.style.top = '0';
          accCanvas.style.width = '100%';
          accCanvas.style.height = '100%';
          if (accCanvas.parentNode !== ensureRootLayer()) { try { ensureRootLayer().appendChild(accCanvas); } catch(e){} }
        }
      }
    } catch (e_pos) {}
  }
  ...
  function resize() {
    ...
    // На некоторых Android TV (Mi Box и др.) innerWidth/innerHeight могут быть 0 на старте или после возврата.
    // Делаем более надёжный расчёт размеров вьюпорта.
    var dw = window.innerWidth || 0;
    var dh = window.innerHeight || 0;

    var de = document.documentElement;
    if ((!dw || dw < 100) && de) dw = de.clientWidth || de.offsetWidth || dw;
    if ((!dh || dh < 100) && de) dh = de.clientHeight || de.offsetHeight || dh;

    if ((!dw || dw < 100) && document.body) dw = document.body.clientWidth || document.body.offsetWidth || dw;
    if ((!dh || dh < 100) && document.body) dh = document.body.clientHeight || document.body.offsetHeight || dh;

    // крайний фоллбек
    if ((!dw || dw < 100) && window.screen) dw = window.screen.width || dw;
    if ((!dh || dh < 100) && window.screen) dh = window.screen.height || dh;
  ...
  function loop(ts) {
    ...
    if (document.hidden && !isAndroidTV()) {
      lastTs = ts;
  ...
  function startEngine() {
    if (running) return;
    // На Android TV/приставках (Mi Box и др.) prefers-reduced-motion может быть включён по умолчанию,
    // из-за чего эффект не стартует. Не блокируем запуск по этому флагу.
    ...
    try {
      if (Lampa.Listener && Lampa.Listener.follow) {
        Lampa.Listener.follow('activity', function (e) {
          ...
          on_allowed_screen = isAllowedActivity(e) || in_details_activity || isFallbackAllowedActivity(e);
